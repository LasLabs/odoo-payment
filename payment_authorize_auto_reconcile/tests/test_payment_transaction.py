...
class TestPaymentTransaction(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.model_obj = self.env['payment.transaction']
    ...
    def test_transaction_created_with_proper_vals(self):
        tx = self.model_obj._authorize_form_get_tx_from_data(self.post_return)
        expect = '%s [%s]' % (self.post_return['x_invoice_num'],
                              self.post_return['x_trans_id'])
        self.assertEqual(
            expect, tx.reference, 
            'Tx reference not set correctly',
        )
        self.assertEqual(
            ''
        )
